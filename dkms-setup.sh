#!/bin/bash
#
# Set up DKMS for gc2607 and patched ipu-bridge.
...
echo "=== Setting up DKMS for GC2607 camera ==="

# Install DKMS if needed
if ! command -v dkms &>/dev/null; then
    echo "Installing dkms..."
    dnf install -y dkms kernel-devel
fi

# Ensure kernel-devel is installed for current kernel
if [ ! -d "/usr/src/kernels/$(uname -r)" ]; then
    echo "Installing kernel-devel for $(uname -r)..."
...
dkms install "gc2607/${GC2607_VER}" -k "$(uname -r)"
...
# Extract upstream version: "6.19.9-200.fc43.x86_64" -> "6.19.9"
...
#!/bin/bash
# Create depmod override so our patched ipu-bridge in extra/ takes
# priority over the stock in-tree module in kernel/
...
dkms install "ipu-bridge-gc2607/${BRIDGE_VER}" -k "$(uname -r)"
...
echo "No more 'sudo ./gc2607-install.sh' needed after kernel updates."
echo ""
